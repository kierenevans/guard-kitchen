...
module Guard
  class Kitchen < Plugin
    def start
      ...
      cmd = Mixlib::ShellOut.new("chef exec kitchen create", :timeout => 10800)
    ...
    def stop
      ...
      cmd = Mixlib::ShellOut.new("chef exec kitchen destroy", :timeout => 10800)
      cmd.live_stream = STDOUT
      cmd.run_command
      begin
       cmd.error!
    ...
    def run_all
      ...
      cmd = Mixlib::ShellOut.new("chef exec kitchen verify", :timeout => 10800)
      cmd.live_stream = STDOUT
      cmd.run_command
      begin
        cmd.error!
    ...
    def run_on_changes(paths)
      ...
      if suites.length > 0
        ...
        cmd = Mixlib::ShellOut.new("chef exec kitchen verify '(#{suites.keys.join('|')})-.+' -p", :timeout => 10800)
        cmd.live_stream = STDOUT
        cmd.run_command
        begin
          cmd.error!
        ...
        cmd = Mixlib::ShellOut.new("chef exec kitchen converge", :timeout => 10800)
        cmd.live_stream = STDOUT
        cmd.run_command
        begin
          cmd.error!
